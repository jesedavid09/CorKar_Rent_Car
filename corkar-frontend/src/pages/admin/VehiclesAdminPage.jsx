...
import { getVehicles, createVehicle, updateVehicle, deleteVehicle } from '../../services/vehicles.service';
  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        ...
        setVehicles(v);
        const cats = [...new Map(v.map(veh => [veh.category.id, veh.category])).values()];
        setCategories(cats);
      } catch {
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">{vehicles.length} vehículos en flota</p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-left border-b border-gray-200">
                    ...
                    <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Categoría</th>
                    <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Placa</th>
                    <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Precio/día</th>
                  ...
                  {filtered.map(v => (
                    <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-4">
                        <p className="font-semibold text-gray-900">{v.marca} {v.modelo}</p>
                        <p className="text-xs text-gray-400">{v.anio} • {v.transmision === 'AUTOMATICA' ? 'Automático' : 'Manual'}</p>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${estadoBadge(v.estado)}`}>
